...
def create_test_repo(path):
    ...
    for version in expected_versions:
        ...
        repo.create_tag(f'test-{version}', ref=version_commit)
...
def test_get_version_patch_bump():
    assert semver.get_latest_version(bump='patch', version='0.0.0') == Version('0.0.3')
    assert semver.get_latest_version(bump='patch', version='0.0.2') == Version('0.0.3')
    assert semver.get_latest_version(bump='patch', version='0.2.0') == Version('0.2.1')
    assert semver.get_latest_version(bump='patch', version='0.2.100') == Version('0.2.1')
    assert semver.get_latest_version(bump='patch', version='0.2.1') == Version('0.2.1')
    assert semver.get_latest_version(bump='patch', version='9.8.7') == Version('9.8.0')


def test_get_version_minor_bump():
    assert semver.get_latest_version(bump='minor', version='0.0.0') == Version('0.2.1')
    assert semver.get_latest_version(bump='minor', version='0.1.0') == Version('0.2.1')
    assert semver.get_latest_version(bump='minor', version='0.1.100') == Version('0.2.1')
    assert semver.get_latest_version(bump='minor', version='1.0.0') == Version('1.0.0')
    assert semver.get_latest_version(bump='minor', version='2.0.0') == Version('2.0.0')
    assert semver.get_latest_version(bump='minor', version='9.8.7') == Version('9.0.0')


def test_get_version_major_bump():
    assert semver.get_latest_version(bump='major', version='0.0.0') == Version('4.0.0')
    assert semver.get_latest_version(bump='major', version='0.1.0') == Version('4.0.0')
    assert semver.get_latest_version(bump='major', version='0.1.100') == Version('4.0.0')
    assert semver.get_latest_version(bump='major', version='2.0.0') == Version('4.0.0')
    assert semver.get_latest_version(bump='major', version='9.8.7') == Version('4.0.0')


def test_get_version_same_as_latest():
    assert semver.get_latest_version(bump='patch', version='0.1.15') == Version('0.1.15')
    assert semver.get_latest_version(bump='minor', version='1.0.0') == Version('1.0.0')
    assert semver.get_latest_version(bump='major', version='4.0.0') == Version('4.0.0')


def test_can_get_next_version_patch():
    assert semver.get_next_version(bump='patch', version=Version('0.0.0')) == Version('0.0.4')
    assert semver.get_next_version(bump='patch', version=Version('0.1.2')) == Version('0.1.16')
    assert semver.get_next_version(bump='patch', version=Version('0.1.15')) == Version('0.1.16')


def test_can_get_next_version_minor():
    assert semver.get_next_version(bump='minor', version=Version('0.0.0')) == Version('0.3.0')


def test_can_get_next_version_major():
    assert semver.get_next_version(bump='major', version=Version('0.0.0')) == Version('5.0.0')


def test_can_bump():
    with pytest.raises(EnvironmentError):
        semver.can_bump_to(bump='patch', version=Version('0.0.2'))  # Older then latest
        semver.can_bump_to(bump='patch', version=Version('0.0.3'))  # Latest version
...
def test_can_get_current_version():
    ...
    assert semver.get_current_version() == Version('4.0.0')

    assert semver.get_current_version(Version('0.1.15')) == Version('0.1.15')
    assert semver.get_current_version(Version('0.1.2')) == Version('0.1.2')
...
def test_cannot_bump_out_of_order():
    ...
    semver.repo.head.reference = semver.repo.create_head('test-feature', commit='test-0.1.2')
    ...
    assert semver.get_current_version() == Version('0.1.2')
